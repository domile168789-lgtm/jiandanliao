...
class ProfileRepository(
    ...
    suspend fun getContacts(): List<ContactSummary> {
    ...
    suspend fun getGroups(): List<GroupSummary> = listOf(
        GroupSummary(
            name = "柬单聊官方体验群",
            role = "群成员",
            memberCount = 128,
    ...
    suspend fun getDiscoverEntries(): List<DiscoverEntry> = listOf(
        DiscoverEntry(
            title = "活动中心",
    ...
    suspend fun getSecurityOverview(): SecurityOverview {
        val session = ServiceLocator.sessionState.value
